...
class BT_EDD_Log_Table_List extends WP_List_Table {
	...
	public function level_dropdown() {
		...
		$levels = array(
			array( 'value' => 'error',     'label' => __( 'Error',     'bt-edd-mailjet' ) ),
			array( 'value' => 'warning',   'label' => __( 'Warning',   'bt-edd-mailjet' ) ),
			array( 'value' => 'notice',    'label' => __( 'Notice',    'bt-edd-mailjet' ) ),
			array( 'value' => 'info',      'label' => __( 'Info',      'bt-edd-mailjet' ) )
		);

		$selected_level = isset( $_REQUEST['level'] ) ? esc_attr( $_REQUEST['level'] ) : '';
		?>
			<label for="filter-by-level" class="screen-reader-text"><?php _e( 'Filter by level', 'bt-edd-mailjet' ); ?></label>
			<select name="level" id="filter-by-level">
				<option<?php selected( $selected_level, '' ); ?> value=""><?php _e( 'All levels', 'bt-edd-mailjet' ); ?></option>
	...
	public function get_columns() {
		return array(
			'timestamp' => __( 'Timestamp', 'bt-edd-mailjet' ),
			'level'     => __( 'Level',     'bt-edd-mailjet' ),
			'user'     	=> __( 'User',     'bt-edd-mailjet' ),
			'message'   => __( 'Message',   'bt-edd-mailjet' ),
			'source'    => __( 'Source',    'bt-edd-mailjet' ),
		);
	}
	...
	public function column_level( $log ) { 
		$level_key = bt_edd_mailjet()->logger->get_severity_level( $log['level'] );
		$levels    = array(
			'error'     => __( 'Error',     'bt-edd-mailjet' ),
			'warning'   => __( 'Warning',   'bt-edd-mailjet' ),
			'notice'    => __( 'Notice',    'bt-edd-mailjet' ),
			'info'      => __( 'Info',      'bt-edd-mailjet' )
		);
		...
		if( !empty( $log['context'] ) ) {
			...
			if( ! empty( $context['tag_array'] ) ) {
				...
				foreach($context['tag_array'] as $tag_id) {
					$output .= '<li>' . bt_edd_mailjet_get_tag_label( $tag_id ) . '</li>';
				}
	...
	protected function extra_tablenav( $which ) {
		if ( 'top' === $which ) {
			echo '<div class="alignleft actions">';
				...
				$this->user_dropdown();
				submit_button( __( 'Filter', 'bt-edd-mailjet' ), '', 'filter-action', false );
			echo '</div>';
		}
	}
	...
	protected function source_dropdown() {
		...
		$sources_db = $wpdb->get_col( "
			...
			FROM {$wpdb->prefix}bt_edd_logging
			WHERE source != ''
			ORDER BY source ASC
		" );
		...
		if ( ! empty( $sources_db ) ) {
			...
			?>
				<label for="filter-by-source" class="screen-reader-text"><?php _e( 'Filter by source', 'bt-edd-mailjet' ); ?></label>
				<select name="source" id="filter-by-source">
					<option<?php selected( $selected_source, '' ); ?> value=""><?php _e( 'All sources', 'bt-edd-mailjet' ); ?></option>
	...
	protected function user_dropdown() {
		...
		if ( ! empty( $users ) ) {
			...
			?>
				<label for="filter-by-user" class="screen-reader-text"><?php _e( 'Filter by user', 'bt-edd-mailjet' ); ?></label>
				<select name="user" id="filter-by-user">

					<option<?php selected( $selected_user, '' ); ?> value=""><?php _e( 'All users', 'bt-edd-mailjet' ); ?></option>
	...
	public function prepare_items() {
		...
		$per_page = $this->get_items_per_page( 'bt_edd_status_log_items_per_page', 20 );
		...
		$query_count = "SELECT COUNT(log_id) FROM {$wpdb->prefix}bt_edd_logging {$where}";
	...
	protected function get_items_query_where() {
		...
		$where_values     = array();
		if ( ! empty( $_REQUEST['level'] ) && bt_edd_mailjet()->logger->is_valid_level( $_REQUEST['level'] ) ) {
			$where_conditions[] = 'level = %d';
			$where_values[]     = bt_edd_mailjet()->logger->get_level_severity( $_REQUEST['level'] );
		}
